...
return jdbcTemplate.update("insert into userPersonal (id, login, password, phone,name,avatarimages) " + "values(?, ?, ?, ?, ?, ?)",
        new Object[]{userPersonal.getId(),userPersonal.getName(),userPersonal.getPassword(),userPersonal.getName(),userPersonal.getPhone(),
        userPersonal.getName(),userPersonal.getAvatarimages()});


}
public int update(UserPersonal userPersonal){
return jdbcTemplate.update("update userPersonal " + "set login = ?, password = ?, phone = ?,name = ?, avatarimages = 1" +
        "where id = ?", new Object[]{userPersonal.getLogin(),userPersonal.getPassword(),userPersonal.getPhone(),userPersonal.getName()
        ,userPersonal.getAvatarimages(),userPersonal.getId()});
}
}
